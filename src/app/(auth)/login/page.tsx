...
import { Dumbbell, Lock, Mail, ArrowRight, Zap, Loader2 } from 'lucide-react';
...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${origin}/auth/callback?next=${encodeURIComponent(returnUrl)}`,
        },
      });

      if (oauthError) {
        console.warn('OAuth setup fallback:', oauthError.message);
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50 space-y-6">
      {error && (
        <div className="rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-700 border border-rose-200">
          {error}
        </div>
      )}
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 bg-white py-3.5 px-4 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50 active:scale-95 disabled:opacity-50 transition"
      >
        {isGoogleLoading ? (
      ...
      {/* Quick Demo Owner Access */}
      <div className="pt-1">
        <button
          type="button"
          onClick={handleDemoSignIn}
          className="w-full flex items-center justify-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 hover:bg-emerald-100 py-2.5 px-4 text-xs font-bold text-emerald-800 transition"
        >
          <Zap className="h-3.5 w-3.5 text-emerald-600" />
          <span>Quick Demo Owner Sign-In</span>
        </button>
      </div>

      {/* New gym owner onboarding prompt */}
      <div className="pt-2 border-t border-slate-100 text-center">
        ...
        >
          <span>New to Gymora? Setup your gym in 3 minutes</span>
